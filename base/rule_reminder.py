...
## 格式规范
所有协议块标识符必须独占一行，使用 REPLY_START/END、TODO_START/END、{action_block_marker_text} 和 `@SPORE:STOP`。
...
## 任务完成标记
完成后在 REPLY 块中输出总结，然后输出 `@SPORE:STOP`
...
1. **工具调用必须输出 {action_blocks} 块；完成时在 REPLY 块中输出总结，然后输出 @SPORE:STOP**
2. 所有 START/END 标识符必须独占一行并成对出现
3. 回复必须放在 @SPORE:REPLY_START / @SPORE:REPLY_END 块中
4. 任务完成后输出 @SPORE:STOP
