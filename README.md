...
[下载 Release](https://github.com/miunasu/Spore/releases) · [配置说明](docs/CONFIGURATION.md) · [界面指南](docs/FRONTEND.md) · [CLI 模式](docs/CLI.md) · [架构设计](docs/ARCHITECTURE.md) · [构建指南](docs/BUILD.md)
...
而这一切，都装在一个**普通人打开就会用的桌面软件**里：浏览器式标签页、图形化设置、把命令行翻译成人话。不懂技术、不碰终端，也能让 AI 在自己的电脑上真正干活。
...
主机能力如果没有兜底，就是危险品。Spore 的安全体系分三段，覆盖"执行前 / 执行中 / 出事后"：

- **执行前 —— 高危确认**：命中系统级高危规则（服务、注册表、Defender、磁盘、卷影、计划任务、UAC 等）的命令，先由安全 Agent 用**普通人能看懂的语言**说明：这是要干什么、风险多大、是否可逆、怎么回滚，再由你决定继续或取消。
- **执行中 —— 异步研判与熔断**：默认模式下，安全 Agent 在后台旁路研判每条命令的意图与恶意迹象。一旦确认恶意，Spore 会**熔断当前会话、取消在途请求、终止后台子 Agent**。
...
- **和 Windows 原生打通**：右侧文件管理器能和系统资源管理器互相复制粘贴文件、拖拽文件进来编辑 —— 是真桌面软件的手感，不是套壳网页。
...
- 文件写入 / 编辑 / 删除，只在内容**真正变化**时记录版本（内容寻址 + 二进制增量，成本很低）；
- 删错的文件能从历史恢复，改错的能回退到任意版本；
- `rewind` 可以**同时回退文件、对话历史和 TODO**，把整个"这一步做错了"一次性撤销；
...
一键切换成 **380 × 520 的置顶 Mini 悬浮窗**，像迷你音乐播放器一样常驻桌角，不挡你正在用的 IDE、浏览器或文档。它只保留运行期真正需要的东西：最近的回复、当前活动与风险状态、高危确认、快速输入。鼠标靠近底部时输入栏自动浮现，退出 Mini 模式后窗口恢复原来的大小、位置和置顶状态。
...
桌面后端同时提供本地 HTTP 与 WebSocket 服务。即使不用 GUI，你也可以把它接入自己的系统：提交任务、读取历史、管理会话、接收结构化事件、调用回滚。默认监听 `127.0.0.1:8765`(HTTP) 与 `127.0.0.1:8766`(WebSocket)，适合作为本地自动化编排层、内网 Agent 服务或桌面工作流后端。

## 🧠 用得久也不会"忘事"、不会"串台"

- 上下文接近阈值时自动压缩，长对话不崩；
- 短记忆自动保存，应用重开能接着聊；
- 多会话彻底隔离：各自独立的对话循环、日志、任务身份和执行锁，多个标签页互不干扰；
...
文件恢复系统独立于 Git，使用内容寻址与版本记录：写入前后比较内容哈希，仅真实变化才保存；支持版本查看、按版本恢复与按步回退；删除文件与目录中的文件可进入恢复链；增量版本使用 `bsdiff4` 降低存储成本；每隔若干版本保存完整锚点控制恢复链长度；恢复操作自身也会记录版本。对话点回滚会尽可能同步恢复文件、消息历史和 TODO，且带会话身份，避免多会话场景下误伤其它会话。
